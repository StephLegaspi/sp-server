...
exports.getAll = () =>{
  return new Promise((resolve, reject) => {
    const queryString = "SELECT *, CONCAT(DATE_FORMAT(request_timestamp, '%e %b, %Y'),' ', TIME_FORMAT(request_timestamp, '%h:%i:%s')) as request_timestamp2 FROM request_information;"
...
exports.getByStatus = (status) =>{
  return new Promise((resolve, reject) => {
    ...
    if(status==='All'){
      queryString = "SELECT *, CONCAT(DATE_FORMAT(request_timestamp, '%e %b, %Y'),' ', TIME_FORMAT(request_timestamp, '%h:%i:%s')) as request_timestamp2 FROM request_information;"
    }else{
      queryString = "SELECT *, CONCAT(DATE_FORMAT(request_timestamp, '%e %b, %Y'),' ', TIME_FORMAT(request_timestamp, '%h:%i:%s')) as request_timestamp2 FROM request_information WHERE status = '" + status +"';"
    }
...
exports.getOne = (id, status) =>{
  return new Promise((resolve, reject) => {
    ...
    if(status==='All'){
      queryString = "SELECT *, CONCAT(DATE_FORMAT(request_timestamp, '%e %b, %Y'),' ', TIME_FORMAT(request_timestamp, '%h:%i:%s')) as request_timestamp2 FROM request_information WHERE id = '" + id +"';"
    }else{
      queryString = "SELECT *, CONCAT(DATE_FORMAT(request_timestamp, '%e %b, %Y'),' ', TIME_FORMAT(request_timestamp, '%h:%i:%s')) as request_timestamp2 FROM request_information WHERE id = '" + id +"' AND status='" + status +"';"
    }
