...
exports.getAllPurchase = () =>{
  return new Promise((resolve, reject) => {
    const queryString = "SELECT order_information.id, order_information.delivery_address, order_information.zip_code, CONCAT(DATE_FORMAT(order_timestamp, '%e %b, %Y'),' ', TIME_FORMAT(order_timestamp, '%h:%i:%s')) as order_timestamp2, shopping_cart.total_items, shopping_cart.total_bill, order_information.status, order_information.shopping_cart_id, order_information.customer_id FROM order_information, shopping_cart WHERE order_information.shopping_cart_id=shopping_cart.id AND  order_information.for_purchase=1;"
...
exports.getByStatPurchase = (delivery_status) =>{
  return new Promise((resolve, reject) => {
    ...
    if(delivery_status==='All'){
      queryString = "SELECT order_information.id, order_information.delivery_address, order_information.zip_code, CONCAT(DATE_FORMAT(order_timestamp, '%e %b, %Y'),' ', TIME_FORMAT(order_timestamp, '%h:%i:%s')) as order_timestamp2, shopping_cart.total_items, shopping_cart.total_bill, order_information.status, order_information.shopping_cart_id, order_information.customer_id FROM order_information, shopping_cart WHERE order_information.shopping_cart_id=shopping_cart.id AND  order_information.for_purchase=1;";
    }else{
